...
class DAVPipeline(DiffusionPipeline):
    ...
    def single_infer(self, rgb, position_ids=None):
    ...
    def single_interp_infer(self, rgb, masked_depth_latent, mask):
        rgb_latent = self.encode(rgb)
        noise_latent = torch.randn_like(rgb_latent)
    ...
    def __call__(
        ...
        if N <= num_frames or N <= num_interp_frames + 2 - num_overlap_frames:
            depth_latent = self.single_infer(rgb)
        else:
            ...
            key_depth_latent = self.single_infer(
                key_rgb, sorted_key_frame_indices.unsqueeze(0).repeat(B, 1)
            )
